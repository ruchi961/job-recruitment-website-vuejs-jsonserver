...
<div class="box2">
        ...
        <router-view></router-view>
            ...
            <label class="email">Enter Valid Email ID</label>    
        ...
        <img class="passimg" src="./pass.jpg">
            ...
            <img class="cpassimg" src="./cpassin.png">
            <label class="cpass">Confirm Password</label>
            <input type="password"  placeholder="Minimum 8 Characters" class="cpassin" v-model="confirm_password">
            <button  @click="created" class="submit">Update</button>
            
</div>
   
   
    <!--<div v-if="verify" class="proceed"> 
        <video src="./vid.mp4" autoplay loop width="1220px" height="750px"></video>
        <div>
      
     <span class="pro1">Welcome to MyJob.com {{lists.firstname}} </span>
       <span class="pro2">        
        Thank you for choosing us.</span>
     <span class="pro3">Let us together take the first step of your dream job.</span>
     <span class="pro4">Press ok to continue</span>
        
     <button class="pro5" @click="change">ok</button>
...
<p class="format4"><b>Read our</b></p><router-link class="format6" to="/term">Terms and Condition</router-link>
<router-view />
<p class="format7"><b>to Know more about us</b></p>
...
methods:{
    async created(){
     ...
     localStorage.setItem('useremail',this.check_email);
      ...
      console.log(datalist)
      if (i===0) {
          if(this.check_password==this.confirm_password){
              let passplit=this.check_password.split("");
                    console.log(passplit)
                    console.log(passplit.length>8)
                      if(passplit.length>8){
          console.log(i) 
        this.ans=alert("Your password is successfully updated !!\nYou have logged in as well!")
        ...
        this.users.password=this.check_password
        await axios.put("http://localhost:3000/users/"+this.users.id,{...this.users,password:this.users.password,status:this.users.status})
        ...
        this.$router.push("/profile")
            ...
            alert("Password of confirm field should match with the entered password ")
        }
      } else {
          alert("You are not a registered user. \nPlease Register and then login. \nPlease check the email address or Pasword ")
